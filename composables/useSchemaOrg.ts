...
export const useSchemaOrg = () => {
  ...
  const createOrganizationSchema = () => {
    return {
      ...
      description: 'Кадастровые работы в Плесецком районе Архангельской области. Межевание земельных участков, технические планы, постановка на кадастровый учет. Опыт более 15 лет.',
      foundingDate: '2009',
      address: {
        '@type': 'PostalAddress',
        addressLocality: 'Плесецк',
   ...
   */
  const createBreadcrumbSchema = (breadcrumbs: Array<{ name: string; url: string }>) => {
    return {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
  ...
  const createWebSiteSchema = () => {
    return {
      ...
      url: 'https://pleskad.ru',
      description: 'Профессиональные кадастровые услуги в Плесецком районе Архангельской области',
      potentialAction: {
        '@type': 'SearchAction',
        target: {
          '@type': 'EntryPoint',
