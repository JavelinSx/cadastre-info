...
import * as serviceDataModules from '~/utils/data/services';
import type { ServiceData } from '~/utils/types/service-types';

import { useRoute } from 'vue-router';
definePageMeta({
  title: 'Технические планы - Услуги кадастрового инженера',
  description: 'Профессиональная подготовка технических планов зданий, сооружений, помещений и объектов незавершенного строительства. Все виды технических планов от опытных кадастровых инженеров.',
});
// Получаем текущий маршрут
const route = useRoute();

// Карта соответствия URL-путей и ключей данных
const servicePathMap: Record<string, string> = {
  'technical-plans': 'technicalPlans',
  'land-survey': 'landSurvey',
  'cadastral-register': 'cadastralRegister',
  'property-rights': 'propertyRights',
  'error-correction': 'errorCorrection',
  'cadastral-excerpts': 'cadastralExcerpts'
};

// Получаем ключ данных по пути
const getServiceKeyFromPath = (): string => {
  // Получаем последнюю часть URL-пути
  const path = route.path.split('/').pop() || '';
  // Возвращаем соответствующий ключ данных или технические планы по умолчанию
  return servicePathMap[path] || 'technicalPlans';
};

// Получаем данные о сервисе по ключу
const serviceData = computed(() => {
  const serviceKey = getServiceKeyFromPath();
  const key = `${serviceKey}Data` as keyof typeof serviceDataModules;
  return serviceDataModules[key] as ServiceData;
});
</script>
