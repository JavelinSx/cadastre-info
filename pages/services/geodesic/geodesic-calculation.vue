<template>
  ...
  import { geodesicCalculationData } from '~/utils/data/services/geodesic/geodesic-calculation';
  
  definePageMeta({
    title: 'Определение объемов - Геодезические работы',
    description: 'Профессиональные услуги по геодезическому определению объемов земляных масс, выемок, насыпей и строительных материалов с высокой точностью.',
  });
  
  // Используем данные из файла
  const serviceData = ref(geodesicCalculationData);
  </script>
